#! /usr/bin/perl -w
# vi: set ft=perl expandtab shiftwidth=3 tabstop=3:
#
# colorgcc
#
# Version: 1.3.2
...
#       Found STDIN bug.
#
#    <gavinbeatty@optiver.com> (Gavin Beatty)
#       gavinb-1, gavinb-2.
#
# Changes:
#
# gavinb-2: Allow config stuff that starts with (cc|gcc|c++|g++) so g++44 will
#           work.
# gavinb-1: Don't stop STDOUT being colorized if it's not a tty.
#
...
$0 =~ m%.*/(.*)$%;
$progName = $1 || $0;
if ($compilerPaths{$progName}) {
   $compiler = $compilerPaths{$progName};
} else {
   my @PATH_ = split(/:/, $PATH);
   # find second one in PATH
   my $i = 0;
   foreach my $path (@PATH_) {
      if (-x "${path}/$progName") {
         if ($i == 1) {
            $compiler = "${path}/$progName";
            last;
         }
         $i = $i + 1;
      }
